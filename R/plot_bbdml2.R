...
# 
# x <- corncob_da7
# color <- "AvgSiteTempGroup"
  
plot.bbdml2 <- function(x, total = FALSE, color = NULL, shape = NULL, facet = NULL, title = NULL, B = 1000, sample_names = TRUE, data_only = FALSE, ...) {
  ...
  if (!(data_only)) {
    ...
    my_gg <- ggplot2::ggplot(df, aes_map) +
      ...
      ggplot2::geom_errorbar(ggplot2::aes(ymin = ymin, ymax = ymax), width = .75,size=1) +
